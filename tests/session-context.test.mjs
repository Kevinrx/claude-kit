...
function planRepo() {
  ...
  mkdirSync(join(dir, '.claude', 'plans', 'discounts'), { recursive: true });
  writeFileSync(join(dir, '.claude', 'plans', 'discounts', 'PLAN.md'), '# Discounts\n');
  writeFileSync(join(dir, '.claude', 'plans', 'discounts', 'PROGRESS.md'), 'status: in progress\n## Log\n- step 1 — done — bin/rspec green\n');
  mkdirSync(join(dir, '.claude', 'plans', 'old'), { recursive: true });
  writeFileSync(join(dir, '.claude', 'plans', 'old', 'PLAN.md'), '# Old\n');
  writeFileSync(join(dir, '.claude', 'plans', 'old', 'PROGRESS.md'), 'status: done\n');
  return dir;
}
...
test('session-context reports stack, gate and unfinished plans', () => {
  ...
  assert.match(r.stdout, /\.claude\/plans\/discounts/);
  assert.doesNotMatch(r.stdout, /\.claude\/plans\/old/);
...
test('session-context repeats plan progress after compaction', () => {
  ...
  assert.match(r.stdout, /\.claude\/plans\/discounts\/PROGRESS\.md \(last lines\)/);
