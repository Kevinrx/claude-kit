...
Location: the repo's convention if it has one; otherwise `.claude/kit-plans/<slug>/`. If I don't want plans committed in this repo, add `.claude/kit-plans/` to `.git/info/exclude` (local-only — never edit the team's `.gitignore` for this).
...
```
/goal Implement .claude/kit-plans/<slug>/PLAN.md with /kit:implement --auto until: <goal line>
```
