...
1. **Where.** The active plan's `PROGRESS.md`. No plan → create `.claude/kit-plans/<slug>/PROGRESS.md` with a one-line objective.
...
3. **Ask me** whether to commit a WIP (`wip: handoff <slug>`) and push the branch so the other machine can pick it up. Touch nothing in git without a yes. If `.claude/kit-plans/` is in `.git/info/exclude`, the file won't travel with a push — say so, and offer to paste the handoff into the PR or issue instead.
4. **Tell me the resume command**: `/kit:implement <slug>`, or "read `.claude/kit-plans/<slug>/PROGRESS.md` and continue".
