...
3. **Model.** Both agents default to Sonnet — cheap enough for routine diffs. Pass an explicit `model: "opus"` override (to either agent) when the diff is large (rough guide: >150 changed lines or >4 files) or touches a core-risk area with real stakes — auth/session logic itself, payment processing, a migration that moves data, mass deletion — not just any diff that happens to touch a param. `--thorough` forces the Opus override regardless of size.
