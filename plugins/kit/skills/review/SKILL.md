...
3. **Model.** Both agents default to Sonnet — cheap enough for routine diffs (Opus 5.5 still costs ~1.5× per review and found little extra on routine diffs — claude-kit `docs/spikes/opus-5-5-review.md`). Pass an explicit `model: "opus"` override (to either agent) when the diff is large (rough guide: >150 changed lines or >4 files) or touches a core-risk area with real stakes — auth/session logic itself, payment processing, a migration that moves data, mass deletion — not just any diff that happens to touch a param. `--thorough` forces the Opus override regardless of size.
