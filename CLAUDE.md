...
- `setup/setup.mjs` — per-machine setup. `tests/` — `node --test` suite for the hooks.

Rules:
- Hooks must never break a session: wrap logic in `main()` from `hooks/lib.mjs`; only `stop-gate.mjs` exits 2, deliberately.
- Every hook change needs a test in `tests/hooks.test.mjs`. Run `npm test` and `npm run validate` before committing.
